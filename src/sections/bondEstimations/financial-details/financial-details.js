...
export default function FinancialDetails({
  ...
  const calculatedRatios = useMemo(() => {
    ...
    const quickAssets = toNumber(currentFundPosition?.quickAssets);
    const totalAssets = toNumber(currentFundPosition?.totalAssets || currentFundPosition?.currentAssets);
    ...
    return {
      ...
      returnOnAssets:
        totalAssets > 0 && netProfit !== 0 ? formatRatio((netProfit / totalAssets) * 100) : '',
    };
      ...
      <Box display="flex" flexDirection="column" gap={3}>
        <Card sx={{ p: 3, borderRadius: 3, boxShadow: 3 }}>
          ...
          <Typography variant="body2" mb={3}>
            Provide your company key financial ratios to assess financial health
          </Typography>
